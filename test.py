...
def parse_args():
    ...
    parser.add_argument("--run_name", type=str, default="show", help="run model name")
    parser.add_argument("--batch_size", type=int, default=1, help="batch size")
    parser.add_argument("--image_size", type=int, default=256, help="image_size")
    parser.add_argument('--device', type=str, default='cuda:2')
    parser.add_argument("--data_path", type=str, default="data/Refuge", help="train data path") 
    parser.add_argument("--metrics", nargs='+', default=['iou', 'dice'], help="metrics")
    parser.add_argument("--model_type", type=str, default="vit_b_fusion", help="sam model_type")
    parser.add_argument("--moe_start", type=int, default=0, help="moe layers start from")
    parser.add_argument("--sam_checkpoint", type=str, default=None, help="sam checkpoint")
    parser.add_argument("--resume", type=str, default="workdir/models/fusion/epoch2_sam.pth", help="load resume") 
    parser.add_argument("--boxes_prompt", type=bool, default=True, help="use boxes prompt")
    parser.add_argument("--point_num", type=int, default=8, help="point num")
    ...
    parser.add_argument("--mode", type=str, default='test', help="test or train mode")
    parser.add_argument("--dataset_mode", type=str, default='shipan', help="test or train mode")
    parser.add_argument("--experts", type=int, default=4, help="amount of experts")
    parser.add_argument("--rank", type=int, default=48, help="rank")
...
def prompt_and_decoder(args, batched_input, ddp_model, image_embeddings):
    if  batched_input["point_coords"] is not None:
        ...
        points = None
    interm_embeddings = ddp_model.image_encoder.get_interm_embeddings()
    patch_embeddings = ddp_model.image_encoder.get_patch_embeddings()
    with torch.no_grad():
...
def main(args):
    ...
    lora_sam = LoRA_Sam(model,r = args.rank,k=False).to(args.device)
    model = lora_sam.sam
    with open(args.resume, "rb") as f:
        checkpoint = torch.load(f,map_location=args.device)
        model.load_state_dict(checkpoint['model'],strict=True)
        print(f"*******load {args.resume}")
    criterion = FocalDiceloss_IoULoss(weight=1)
    test_dataset = TestingDataset(data_path=args.data_path, image_size=args.image_size, mode=args.dataset_mode, requires_name=True, point_num=args.point_num, return_ori_mask=True, prompt_path=args.prompt_path,label=9)
    ...
    for i, batched_input in enumerate(test_pbar):
        ...
        with torch.no_grad():
            image_embeddings = model.image_encoder(batched_input["image"],batched_input["token"])
    ...
    #    routing_dict[i]= model.image_encoder.blocks[i].attn.routing.label_list
    # df = pd.DataFrame(routing_dict)
