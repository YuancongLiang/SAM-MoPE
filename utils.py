...
def get_boxes_from_mask(mask, box_num=1, std = 0.1, max_pixel = 5):
    ...
    if len(boxes)==0:
        boxes= [find_bounding_box(mask)] # 这边append不上去，所以直接赋值
...
def find_bounding_box(mask):
    ...
    # 返回矩形框的坐标
    return (top_left[1], top_left[0], bottom_right[1], bottom_right[0])  # (x1, y1, x2, y2)
...
class FocalDiceloss_IoULoss(nn.Module):
    ...
    def __init__(self, weight=20.0, iou_scale=0.0):
